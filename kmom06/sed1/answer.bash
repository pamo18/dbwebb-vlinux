...
#
# 1f5757821cb06fbad85df34c3666b817
# vlinux
# sed1
# v1
# pamo18
...
# v4.0.0 (2019-03-05)
#
# Generated 2019-10-07 14:30:47 by dbwebb lab-utility v4.0.0 (2019-03-05).
...
ANSWER=$( sed -n -E '/192\.168\.0\.1/p' < numbers.txt )
...
ANSWER=$( sed -n -E '/^The/p' < quotes.txt )
...
ANSWER=$( sed -n -E '/[a-z]{2}\sa/p' < quotes.txt )
...
ANSWER=$( sed -n -E '/e[A-Za-z]*.$/p' < quotes.txt )
...
ANSWER=$( sed -n -E "/[A-Za-z0-9!#$%&\'\"*+-/=?^_\`{|}~]@[A-Za-z0-9]*\.[a-z]{2}/p" < emails.txt )
...
ANSWER=$( sed -n -E 's/Thor/MightyThor/p' < substitution.txt )
...
ANSWER=$( sed -n -E 's/home/Asgaard/p' < substitution.txt )
...
ANSWER=$( sed -n -E 's/Thor/MightyThor/;s/home/Asgaard/p' < substitution.txt )
...
ANSWER=$( sed -E '/\/home\/Thor\/Pictures\/Family\/$/a /home/Thor/Pictures/Family/Thor/selfie.png' < substitution.txt )
...
ANSWER=$( sed -n -E 's/.*([0-9]{2})\/([A-Za-z]{3})\/([0-9]{4}).*/\3-\2-\1/p' < access-medium.log )
...
ANSWER=$( sed -n -E 's/^([4-6][0-9]?[0-9]?\.[0-9][0-9]?[0-9]?\.[0-9][0-9]?[0-9]?\.[0-9][0-9]?[0-9]?).*/\1/p' < access-medium.log )
...
ANSWER=$( sed -n -E 's/.*(http[a-z]?:\/\/[a-z0-9]+\.[a-z0-9]+\.?[a-z0-9]+?\.?[a-z0-9]+?).*/\1/p' < access-medium.log)
